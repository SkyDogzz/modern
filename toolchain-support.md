...
runner family, an immutable official Ubuntu 24.04 container digest, explicit
compiler/package majors, Ninja, and a commit-pinned checkout action.
[`support/toolchain-profile.cpp`](support/toolchain-profile.cpp) makes the job fail
unless the GCC 14 job resolves libstdc++ 14 and the Clang 18 job resolves libc++ 18.
The workflow records the GitHub host image and resolved package versions. Package
patches from the configured Ubuntu repositories remain security-updatable, while
the execution userspace base is digest-pinned; archive exact package artifacts as
well when bit-for-bit reconstruction is required.
